...
$key = Get-UninstallRegistryKey -SoftwareName 'AuthGeek*'
...
elseif ($key.Count -eq 0) {
  Write-Warning 'AuthGeek is not installed, or was installed outside of Chocolatey. Nothing to do.'
}
else {
  Write-Warning "$($key.Count) matches found for AuthGeek. Remove it manually rather than guessing:"
