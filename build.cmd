@echo off
rem AuthGeek local build. Mirrors .github\workflows\release.yml so a local artefact and a
...
dotnet build AuthGeek.sln -c Release || exit /b 1
dotnet run --project tests\AuthGeek.Tests -c Release --no-build || exit /b 1

if /i "%~1"=="" goto :eof

dotnet publish src\AuthGeek\AuthGeek.csproj -c Release -r win-x64 ^
...
"%ISCC%" installer\AuthGeek.iss || exit /b 1
